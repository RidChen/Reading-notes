...
@implementation UIViewController (Tracing)

// 测试method Swizzling
+(void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        swizzleMethod([UIViewController class], @selector(viewDidLoad), @selector(trackingViewDidLoad));
    });
}
...
-(void)trackingViewDidLoad {
    // 添加的代码
    NSLog(@"tracking viewDidLoad");
    return [self trackingViewDidLoad];
}
